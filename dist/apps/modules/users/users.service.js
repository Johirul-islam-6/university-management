...
const createdStuden = (student, user) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // set role genarate
    user.role = 'student';
    const academecsemester = yield academicSemester_model_1.AcademicSemester.findById(student.academicSemester);
    // 01--> session start
    ...
    try {
        ...
        if (!newStudent.length) {
            throw new ApiError_1.ApiError(http_status_1.default.BAD_REQUEST, 'fail to create student', '');
        }
        //03. sessiot create a studen
        user.student = newStudent[0]._id;
        const newUser = yield users_model_1.User.create([user], { session });
        if (!newUser.length) {
            throw new ApiError_1.ApiError(http_status_1.default.BAD_REQUEST, 'fail to create user', '');
        }
